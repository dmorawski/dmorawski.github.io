...
// Function to find a record by part number
function findRecordByPartNumber(records, partNumber) {
    return records.find(record => record["Manufacturer Part Number"] === partNumber);
}

async function generateQuote() {
    const csvFilePath = "./pricing.csv"; // Path to your CSV file
    ...
    for (let i = 1; i <= operatorCount; i++) {
        ...
        const record = findRecordByPartNumber(records, operatorPartNumber);
        const shortCode = record?.["Short Number"] || "N/A";
        const price = parseFloat(record?.["Price"] || 2300);
    ...
    ["switches", "powerSupplies", "bollards"].forEach(category => {
        const categoryInputs = document.querySelectorAll(`.${category}-quantity`);
        categoryInputs.forEach(input => {
            const partNumber = input.dataset.partNumber;
            const quantity = parseInt(input.value) || 0;
            if (quantity > 0) {
                const record = findRecordByPartNumber(records, partNumber);
                const shortCode = record?.["Short Number"] || "N/A";
    ...
    // Sort parts list to ensure labor is last
